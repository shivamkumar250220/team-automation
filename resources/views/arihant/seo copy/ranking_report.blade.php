...
@section('title', 'Ranking Report Auto-pull')
@section('page_header', 'Ranking Report Auto-pull')
...
<div class="row g-4">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">Ranking Report Auto-pull</h5>
                <p class="card-subtitle">Configure location, keywords and competitor analysis options</p>

                <form id="rankingReportForm" novalidate>
                    @csrf
                    <div class="row g-4">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="keywordsInput" name="keywords"
                                    placeholder="Enter multiple keywords separated by commas (e.g., SEO, marketing, web design)" required>
                                ...
                                <div id="competitorDomainsInput" style="display: none;">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="competitorUrls" name="competitor_urls"
                                            placeholder="Enter competitor URLs separated by commas (e.g., https://example.com, https://competitor.com)">
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h5 class="card-title mb-0">Search Ranking Results</h5>
                ...
                {{-- Nav Tabs for multiple keywords --}}
...
// ─── Form submit ──────────────────────────────────────────────────────────────
document.getElementById('rankingReportForm').addEventListener('submit', async function (e) {
    e.preventDefault();
    ...
    let valid = true;
    if (!location) {
        document.getElementById('locationSelect').classList.add('is-invalid');
        valid = false;
    } else {
        document.getElementById('locationSelect').classList.remove('is-invalid');
    }
    if (!keywords) {
        document.getElementById('keywordsInput').classList.add('is-invalid');
        valid = false;
    } else {
        document.getElementById('keywordsInput').classList.remove('is-invalid');
    }
    if (!valid) return;
...
function renderResults(results, location, competitorList, clientDomain, competitorMode) {
    ...
    const tabsEl   = document.getElementById('keywordTabs');
    const contentEl = document.getElementById('keywordTabsContent');
    tabsEl.innerHTML = '';
    contentEl.innerHTML = '';

    results.forEach((res, idx) => {
        const safeName = 'kw_' + idx;
        const isFirst  = idx === 0;
        ...
        const buildOrganic = (rows) => rows.map(r => {
        ...
        const buildCompetitor = (rows) => rows.map(r => {
            const roleBadge = '<span class="keyword-badge competitor-tag">Competitor</span>';
            return `
            <tr>
                <td><span class="position-badge ${posClass(r.position)}">${r.position ?? '—'}</span></td>
        ...
        pane.innerHTML = `
            ...
            <div class="table-responsive mb-5">
                <table id="${orgTableId}" class="table table-hover align-middle w-100">
                    <thead class="table-light">
                        <tr>
                            <th>Position</th><th>Title</th><th>URL</th><th>Domain</th><th>Snippet</th><th>Role</th>
                        </tr>
                    </thead>
                    <tbody>${buildOrganic(allRows)}</tbody>
                </table>
            </div>
            ...
            <div class="table-responsive">
                <table id="${compTableId}" class="table table-hover align-middle w-100">
                    <thead class="table-light">
                        <tr>
                            <th>Position</th><th>Title</th><th>URL</th><th>Domain</th><th>Role</th>
                        </tr>
                    </thead>
        ...
        setTimeout(() => {
            dtInstances[orgTableId] = $(`#${orgTableId}`).DataTable({
                pageLength: 25,
                order: [[0, 'asc']],
            ...
            dtInstances[compTableId] = $(`#${compTableId}`).DataTable({
                pageLength: 25,
                order: [[0, 'asc']],
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function buildRows(data, competitorList) {
    if (!data) return [];
...
function isCompetitor(link, list) {
    if (!link || !list.length) return false;
    return list.some(c => link.toLowerCase().includes(c.toLowerCase().replace(/https?:\/\/(www\.)?/, '')));
}

function domainMatch(link, input) {
    if (!link || !input) return false;
    const normalize = s => s.toLowerCase().replace(/https?:\/\/(www\.)?/, '').replace(/\/$/, '').split('/')[0];
    return normalize(link).includes(normalize(input)) || normalize(input).includes(normalize(link));
}
...
document.getElementById('exportCsvBtn').addEventListener('click', function () {
    const activePane = document.querySelector('.tab-pane.active table');
    ...
    a.download = `ranking_report_${Date.now()}.csv`;
    a.click();
});

// Snippet modal — delegated listener (works for dynamically created rows)
document.getElementById('keywordTabsContent').addEventListener('click', function (e) {
